                        ...
                        <div class="wsus__dashboard_profile">
                            <div class="wsus__dash_pro_area">
                                            ...
                                            <div class="gallery gallery-md">
                                                @foreach ($productImages as $productImage)
                                                    <div style="display:inline-block; height: 100px ;width:100px; background: url({!! asset('uploads/' . $productImage->name) !!}); "
                                                        class="mx-1 my-1">
                                                        <button id="{{ $productImage->id }}"
                                                            class="fas fa-times-circle delete-image"
                                ...
                                <form class="dropzone mb-3" method="POST" id='myDropzone' enctype="multipart/form-data"
                                    action="{{ route('vendor.product.upload.images', $product->product_key) }}">
                                    @csrf
                                </form>
                                            ...
                                            <div class="form-group">
                                                ...
                                                <select name="category_id" id=""
                                                    ...
                                                    @foreach ($categories as $category)
                                                        <option value="{{ $category->id }}" @selected(old('category_id', $category->id) == $product->category_id)>
                                                            {{ $category->name }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                                    ...
                                                    @if (isset($subCategories))
                                                        @foreach ($subCategories as $subCategory)
                                                            <option value="{{ $subCategory->id }}"
                                                                @selected(old('sub_category_id', $subCategory->id) == $product->sub_category_id)>
                                                    ...
                                                    @if (isset($childCategories))
                                                        @foreach ($childCategories as $childCategory)
                                                            <option value="{{ $childCategory->id }}"
                                                                @selected(old('sub_category_id', $childCategory->id) == $product->child_category_id)>
